...
void load_data(struct Math *math) {
    ...
    // Go back to the start of the input to read in the numbers
    rewind(stdin);
    ...
    while (fgets(line, sizeof(line), stdin) != NULL) {
        ...
        // Break the loop when we reach the symbols ros
        ...
        for (size_t col = 0; col < math->col_count; col++) {
            ...
            width = math->col_len[col];
            str_idx = 0; // Start counting a new string

            // String in column will be the size of the column width
            for (size_t s = 0; s < width; s++) {
                // Current character from the input string
                char ch = line[start + s];

                // Add the character to the new string in the right place
                // Change any spaces to dots to preserve paddings visually
                str[str_idx++] = (ch == ' ') ? '.' : ch;
            }
            ...
            // Set the starting index of the next column within the input string
            start = start + width + 1;
        }
        row++;
    }
...
long long str_to_int(char str[5]) {
    while (*str == '.') str++;
    return atoll(str);
}
...
long long solve_part1(struct Math *math) {
    ...
    for (size_t col = 0; col < math->col_count; col++) {
        ...
        for (size_t row = 0; row < math->row_count; row++) {
            ...
            long long nr = str_to_int(math->numbers[row][col]);
...
long long solve_part2(struct Math *math) {
    ...
    for (size_t col = 0; col < math->col_count; col++) {
        ...
        // Grab the arithmetic symbol for this column
        char col_symbol = math->symbols[col];
        ...
        for (int col_char = math->col_len[col] - 1; col_char >= 0; col_char--) {
            ...
            // length 4 for the string, 1 for the null terminator
            char str[5];

            // Define an idx variable to track the current new string
            // character position
            size_t idx = 0;
            ...
            for (size_t row = 0; row < math->row_count; row++) {
                ...
                // Increment the string index counter at the same time.
                // ++idx would increment before using the variable
                // idx++ uses the variable before incrementing.
                str[idx++] = ch;
            }
            // Null terminate the string
            str[idx] = '\0';
            long long nr = atoll(str); // Convert the new string to long long

            // Perform the arithmetic operation and 
            // add the result to the total for this column
            col_total = do_op(col_total, nr, col_symbol);
        }
        total += col_total;
    }
    return total;
